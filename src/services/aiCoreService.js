...
export async function requestAnthropicResponse({
  ...
  apiUrl =
    process.env.ANTHROPIC_API_URL || DEFAULT_ANTHROPIC_MESSAGES_URL,
  apiVersion =
    process.env.ANTHROPIC_API_VERSION || DEFAULT_ANTHROPIC_API_VERSION,
  input,
...
export function hasConfiguredAiProvider(env = process.env) {
  return ["openai", "gemini", "grok", "anthropic"].some((provider) =>

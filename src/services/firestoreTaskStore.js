...
const DEFAULT_COLLECTION = "synchron-tasks-v1";

function collectionName(env) {
  const value = String(
    env.FIRESTORE_TASK_COLLECTION || DEFAULT_COLLECTION,
  ).trim();
  if (!/^[A-Za-z0-9_-]{1,120}$/u.test(value)) {
    const error = new Error("Невалидна Firestore task collection.");
...
export function createFirestoreTaskStore({
  ...
  return Object.freeze({
    ...
    listByOwner(ownerHash, limit = 1_000) {

...
const { setFirestoreGitHubSessionStoreForTests } =
  await import("../src/services/githubOAuthService.js");

const githubRecords = new Map();
setFirestoreGitHubSessionStoreForTests({
  async get(id) {
    return githubRecords.has(id) ? structuredClone(githubRecords.get(id)) : null;
  },
  async set(id, payload) {
    githubRecords.set(id, structuredClone(payload));
  },
  async delete(id) {
    githubRecords.delete(id);
  },
});
